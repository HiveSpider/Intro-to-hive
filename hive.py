...
class game():
    ...
    def reveal_stacks(self, run_time = 1):
        print ('jth3')
        return self.set_stacks(7, run_time)
    ...
    def get_coords_from_position(self,position):
        ...
        return coords
    def move(self, piece, position, run_time=1.0, curve_dir=0):
        coords = None
    ...
    def make_moves(self, moves, run_time = 1):
        ret = []
        for move in moves:
            s = move.split(' ')
            piece = s[0]
            position = s[1] if len(s) > 1 else ""
            self.move(piece, position, run_time)
